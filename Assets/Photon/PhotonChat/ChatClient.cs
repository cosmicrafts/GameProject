...
namespace Photon.Chat
{
    ...
    public class ChatClient : IPhotonPeerListener
    {
        ...
        public const int DefaultMaxSubscribers = 1000;
        ...
        /// <summary>Sets the channel properties.</summary>
        ...
        {
            if (!this.CanChatInChannel(channel))
            {
                if (this.DebugOut >= DebugLevel.ERROR)
                {
                    this.listener.DebugReturn(DebugLevel.ERROR, "SetProperties called while not connected to front end server.");
                }
                return false;
            }
            ...
            Dictionary<byte, object> parameters = new Dictionary<byte, object>
            {
                { ChatParameterCode.Channel, channel },
                { ChatParameterCode.Properties, properties },
                { ChatParameterCode.Broadcast, true },
            };

			if (expectedProperties != null && expectedProperties.Count > 0)
			{
				parameters[ChatParameterCode.ExpectedProperties] = expectedProperties;
			}
        ...
        void IPhotonPeerListener.OnOperationResponse(OperationResponse operationResponse)
        {
            switch (operationResponse.OperationCode)
            {
                case (byte)ChatOperationCode.Authenticate:
                    ...
                    break;
                case (byte)ChatOperationCode.SetProperties:
                    if ((operationResponse.ReturnCode != 0) && (this.DebugOut >= DebugLevel.ERROR))
                    {
                        if (operationResponse.ReturnCode == -2)
                        {
                            // False report. This is returned also if you try to delete a non-existing key
                        }
                        else
                        {
        ...
        {
            if(eventData.Parameters.TryGetValue(ChatParameterCode.Properties, out object properties))
            {
                string channel = (string)eventData.Parameters[ChatParameterCode.Channel];

                if(PublicChannels.TryGetValue(channel, out ChatChannel publicChannel))
                {
                    publicChannel.ReadProperties(properties as Dictionary<object, object>);
					this.listener.OnPropertiesChanged(channel, properties as Dictionary<object, object>);
                }

                if(PrivateChannels.TryGetValue(channel, out ChatChannel privateChannel))
                {
                    privateChannel.ReadProperties(properties as Dictionary<object, object>);
					this.listener.OnPropertiesChanged(channel, properties as Dictionary<object, object>);
                }
            }
        }
        ...
        public bool Subscribe(string channel, int lastMsgId = 0, int messagesFromHistory = -1, ChannelCreationOptions creationOptions = null)
        {
            ...
            Dictionary<object, object> properties = creationOptions.Properties;
