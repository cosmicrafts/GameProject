    ...
    public class CandidApiManager : MonoBehaviour
    {
        ...
        private async UniTask InitializeCandidApis(IAgent agent, bool asAnon = false)
        {
            ...
            else
            {
                ...
                if (userNodeIdResult.Tag == Candid.WorldHub.Models.ResultTag.Err)
                {
                    ...
                    if (userNodeIdResult.Tag == Candid.WorldHub.Models.ResultTag.Err)
                    {
                        throw new Exception(userNodeIdResult.AsErr());
                    }
                }
        ...
        private async UniTask FetchConfigs()
        {
            try
            {
                ...
                {
                    throw new Exception(configsByWolrdResult.AsErr());
                }
                ...
                {
                    DataTypes.Action a = new DataTypes.Action(e);
                    return a;
                }).ToArray());
        ...
        private async UniTask FetchToken(FetchDataReq<DataTypes.Token> req)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception($"optional value of {nameof(FetchDataReq<DataTypes.Token>)} cannot be null, it must have a string Token Canister Id value");
                }
                ...
                {
                    throw new Exception(getLoginDataResult.AsErr());
                }
        ...
        private async UniTask FetchNfts(NftCollectionToFetch req)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception($"collectionId value of {nameof(FetchDataReq<DataTypes.NftCollection>)} cannot be an empty string, specify some CollectionId");
                }

                var collection = await GetNfts(collectionId, req.name, req.description, req.urlLogo) ?? throw new Exception($"collection value of {nameof(FetchDataReq<DataTypes.NftCollection>)} is Null");
...
//#endif
        ...
        private async UniTask GetPagedRegistry(Extv2BoomApiClient api, DataTypes.NftCollection collection, uint index)
        {
            ...
            try
            {
                ...
                List<UniTask<string>> asyncTokenIdFunctions = new List<UniTask<string>>();
                List<UniTask<OptionalValue<Extv2Boom.Models.Metadata>>> asyncMetadataFunctions =
                    new List<UniTask<OptionalValue<Extv2Boom.Models.Metadata>>>();
        ...
        private async UniTask FetchBoomDaoNfts(NftCollectionToFetch req)
        {
            ...
            try
            {
                ...
                var collection = await GetBoomDaoNfts(collectionId, req.name, req.description, req.urlLogo) ?? throw new Exception($"collection value of {nameof(FetchDataReq<DataTypes.NftCollection>)} is Null");
...
//#endif
        ...
        private async UniTask FetchListings()
        {
            ...
            Extv2BoomApiClient collectionInterface = new Extv2BoomApiClient(getAgentResult.AsOk(),
                Principal.FromText(Env.Nfts.BOOM_COLLECTION_CANISTER_ID));

            var listingResult = await collectionInterface.Listings();

            List<DataTypes.Listing> listing = new List<DataTypes.Listing>();
            ...
            foreach (var item in listingResult)
            {
                ...
                listing.Add(new DataTypes.Listing(tokenIdentifier, item));
            }
