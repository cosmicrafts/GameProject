...
public class BotEnemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MaxCostUnit = GameCards.Max(f => f.DefaultCost);
        MinCostUnit = GameCards.Min(f => f.DefaultCost);
    ...
    IEnumerator IA()
    {
        ...
        while(!GameMng.GM.IsGameOver())
        {
                ...
                case BotMode.Aggressive: //Select the most expensive card
                    {
                        ...
                        SelectedUnit = GameCards.FirstOrDefault(f => f.DefaultCost <= MaxCostUnit);
                    }
                    break;
                case BotMode.Pasive: //Select the cheapest card
                    {
                        SelectedUnit = GameCards.FirstOrDefault(f => f.DefaultCost <= MinCostUnit);
                    }
                    break;
                default: //Select a random card
                    {
                        for(int i=0; i<10; i++)
                        {
                            ...
                            if (SelectedUnit.DefaultCost <= CurrentEnergy)
                            {
                                break;
                            }
                        }
            ...
            if (SelectedUnit.DefaultCost <= CurrentEnergy)
            {
                ...
                CurrentEnergy -= SelectedUnit.DefaultCost;
            }
        }
    }
}
